...
def init_ledger():
    ...
    if not os.path.exists(LEDGER_FILE):
        ...
        with open(LEDGER_FILE, mode='w', newline='') as file:
            ...
            writer.writerow(["Date", "Action", "Ticker", "Price", "Quantity", "Reason", "Est_Tax_Impact"])
    else:
        ...
        if "Est_Tax_Impact" not in df.columns:
            ...
            cols = ["Date", "Action", "Ticker", "Price", "Quantity", "Reason", "Est_Tax_Impact"]
            df = df[cols]
            df.to_csv(LEDGER_FILE, index=False)

def record_trade(action, ticker, price, quantity, reason, tax_impact=0.0):
    """
    Logs a trade (BUY or SELL) into the CSV with quantity and tax impact.
    """
    date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    
    with open(LEDGER_FILE, mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([date, action, ticker, price, quantity, reason, tax_impact])
    
    logger.info(f"📝 Ledger Updated: {action} {quantity} {ticker} @ ${price:.2f} (Tax: ${tax_impact:.2f})")
